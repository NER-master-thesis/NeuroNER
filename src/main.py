        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                try:
                    ...
                    #    number_of_experiment = parameters['cross_validation']
                    #    chunks = dataset.get_chunks(number_of_experiment)
                    #    originaldataset = dataset.get_copy()
                    #    dataset_filepaths['valid'] = "x"

                    #else:
                    #    number_of_experiment = 1
                    #for cv in range(number_of_experiment):

                        #if 'cross_validation' in parameters and parameters['cross_validation'] > 1:
                        #    dataset = originaldataset.split(chunks, cv)
                        #    print("======= Cross validation n ", cv)
